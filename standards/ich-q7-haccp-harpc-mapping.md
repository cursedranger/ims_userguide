...
- ~~Gate the role assignment on holding the required competency.~~ **Built
  2026-08-06** (architecture.md §11.5.3) — a `blocking` `CompetencyRequirement`
  now refuses a role assignment while the person does not hold the competency,
  validated on `UserRole` so RailsAdmin cannot bypass it. This is what makes
  the roles above worth adding: a `pcqi` role that anyone can be handed is not
  a PCQI requirement, and 21 CFR 117.180's "successful completion of a
  standardized curriculum" is now expressible as a `Competency` the role
  demands.
