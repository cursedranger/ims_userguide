...
**Current overall position, stated plainly: this application is not 21 CFR
Part 11 compliant today.** It has unusually strong foundations for one — a
generic approval engine, PaperTrail on 175 of 196 models, immutable approved
document revisions, CanCanCan on every path, and a real signature model in
Permit to Work — but it is missing the two things Part 11 is fundamentally
*about*: signature manifestation with meaning (§11.50) and a two-component
signing challenge (§11.200(a)). Those are the top of the gap list.
...
| 11.10(i) | Persons who develop, maintain, or use the system have the education, training, and experience to perform their assigned tasks | `CompetencyRequirement` / `TrainingSession` / `TrainingAttendance` / `AssessmentAttempt` (§11.5) | **Partial — Procedural** | The app can *hold* the training records, including validated attendance and a frozen assessment score, which is more than most systems offer. What it does not do is *gate itself*: a user can be granted a role without any check that they hold the competency for it, and there is no system-specific training curriculum modelled. Also nothing covers the developers/maintainers of the system, which this clause explicitly includes. |
...
| § | Requirement | Application module(s) | Status | Notes |
|---|---|---|---|---|
| 11.50(a)(1) | Signed record shows the **printed name** of the signer | `ApprovalStep#approver`; `WorkPermitSignature#signatory_name`; `shared/_approval_history` | **Partial** | The name renders because it is joined from the live `User` row. Part 11 wants the printed name *on the signed record*; a name resolved by association changes retroactively if the user is renamed, and disappears if the user row is removed. `WorkPermitSignature` handles this better — it has a `signed_name` fallback column. Generalize that. |
| 11.50(a)(2) | Signed record shows the **date and time** of signing | `ApprovalStep#acted_at`; `WorkPermitSignature#signed_at` (server-set via `Time.current` in `WorkPermits::Sign`) | **Covered** | Server-generated, never client-supplied — correct. `config.time_zone` defaults to UTC with an ENV override; the validation package should state the time source and that it is NTP-synchronised. |
| 11.50(a)(3) | Signed record shows the **meaning** of the signature (review, approval, responsibility, authorship) | `WorkPermitSignature#declaration` + `#authority` + `#stage` (§11.22) | **Not covered generally — Covered in PTW only** | This is the clearest gap in the whole document. `ApprovalStep` records *that* someone approved, with `stage_key` and an optional comment — but no declaration text and no signature meaning. Permit to Work already does exactly the right thing and for exactly the right reason: it stores the declaration **as rendered at signing time**, because "a signature means the words the signer actually read." That design needs to be lifted into the generic approval engine. See gap G1. |
| 11.50(b) | The above items are subject to the same controls as electronic records, and are included in any human-readable form of the record (display and print) | `shared/_approval_history` partial; `Documents::CoverSheet`; report PDFs (§15.3) | **Partial** | Approval history is displayed and printed, but it will only satisfy §11.50(b) once (a)(1) and (a)(3) actually exist to be printed. |
...
| 11.200(a)(1) | Non-biometric signatures employ **at least two distinct identification components** (e.g. ID code and password) | Devise session login (`database_authenticatable`) | **Not covered — highest-priority gap** | Signing in this app is a single authenticated click. There is no re-authentication at the point of approval anywhere — `grep valid_password?` across `app/` returns nothing. This fails §11.200(a)(1) outright for every approval, and it is the requirement Part 11 exists to impose. See gap G1. |
| 11.200(a)(1)(i) | When several signings are executed during a **single continuous period of controlled system access**, the first signing uses all components; subsequent signings use at least one component executable only by the signer | — | **Not covered** | Requires the "continuous session" concept: a signing session with a defined idle expiry, distinct from the 30-minute Devise `timeout_in`, after which the full two-component challenge returns. |
| 11.200(a)(1)(ii) | Signings **not** performed in a single continuous period use all signature components | — | **Not covered** | Follows from the above. |
| 11.200(a)(2) | Signatures are used only by their genuine owners | §11.10(d) access controls; `SelfApprovalError`; §11.10(j) accountability policy | **Partial — Procedural** | The technical side rests on §11.200(a)(1), which is not built. Password-sharing prevention is ultimately procedural and depends on the §11.10(j) policy. |
| 11.200(a)(3) | Administered and executed such that attempted use by anyone other than the genuine owner requires collaboration of two or more individuals | Devise `lockable` (10 attempts / 1 hour); `must_change_password` on new accounts; no admin password-set-in-the-clear path | **Partial** | Password reset goes through an emailed token to the account owner rather than an admin setting a known password, which is the important control here and it is correct. Weak points: `config.password_length = 6..128` is far below any current expectation and makes guessing plausible; and there is no separate signing credential, so an admin with database access is a single individual who can act. |
...
| 11.300(d) | Transaction safeguards to prevent unauthorized use of passwords/codes, and to detect and report in an immediate and urgent manner any attempted unauthorized use to system security, and as appropriate to organizational management | Devise `lockable` + `failed_attempts` + `locked_at`; `trackable` (`current_sign_in_at/ip`, `last_sign_in_at/ip`, `sign_in_count`) | **Partial** | Detection exists in the data. **Reporting does not** — nothing notifies anyone when an account locks, and the `Notification` pipeline (§5.3) that already mails every assignment and due date is not wired to security events at all. "Immediate and urgent manner" is explicit in the regulation. This is one of the cheapest gaps to close: a security-event notification on lockout, on repeated failures, and on privilege change. See gap G8. |
...
**§11.200(a)(1), §11.50(a)(3), §11.50(a)(1) · Blocking**

The generic approval engine (§5.2) records an approval as an authenticated click
with an optional comment. Part 11 requires a signature: a re-authentication
challenge at the moment of signing, a stored declaration stating what the
signature *means*, and a snapshot of the signer's printed name.

The design already exists in this codebase — `WorkPermitSignature` stores the
rendered `declaration`, an `authority`, a `stage`, a `signed_name` fallback, and
a server-set `signed_at`, with the comment explaining exactly why the declaration
is stored rather than referenced. The work is to generalize that into the
approval engine rather than invent something:

- A signature-meaning vocabulary per approval stage (authored / reviewed /
  approved / verified / responsibility-accepted), configurable per approvable
  type, rendered into a declaration at signing time and frozen on the step.
- Password re-entry in `Approvals::Approve` and `Approvals::Reject`, with a
  continuous-signing-session window (§11.200(a)(1)(i)) that is distinct from the
  Devise 30-minute session timeout, and a full challenge outside it.
- `signed_name` and `signed_title` snapshot columns on `ApprovalStep`.
- Failed signing attempts logged as security events, and counted toward lockout.
- A Part 11 mode flag so non-GxP approvals (BBS actions, visitor passes) are not
  burdened with a password prompt — scope this by approvable type, using the
  in-scope list at the top of this document.
...
- A system-training curriculum in §11.5, with role assignment gated on holding
  the required competency — the competency machinery already exists and this
  is mostly wiring.
